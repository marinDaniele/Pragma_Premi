 ...
 * Data: 2015-06-11
 * Descrizione: Test di unità TU57 Premi::Front-End::Controllers::PresentationViewerController metodo "jumpToNode"
 */
'use strict';
describe('TU57 Premi::Front-End::Controllers::PresentationViewerController metodo "jumpToNode"', function(){
	...
	beforeEach(function(){
		...
		$scope.jumpToNode(node0._id);
	});

	//test
	it("Dovrebbe invocare il metodo $scope.$broadcast, con i parametri 'presentation-goToId' e nodeId", function(){
		expect($scope.$broadcast).toHaveBeenCalledWith('presentation-goToId',node0._id);
	});

	it("Dovrebbe impostare il valore dell'attributo $scope.linearPresentation a false", function(){
		expect($scope.linearPresentation).toBe(false);
	});
});
