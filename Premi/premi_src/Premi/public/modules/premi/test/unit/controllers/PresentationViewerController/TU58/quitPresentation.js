/**
 * File: quitPresentation.js
 * @author gmidena - rockstar249@gmail.com
 * Data: 2015-06-11
 * Descrizione: Test di unità TU58 Premi::Front-End::Controllers::PresentationViewerController metodo "quitPresentation"
 */
'use strict';
describe('TU58 Premi::Front-End::Controllers::PresentationViewerController metodo "quitPresentation"', function(){
	var $scope, $rootScope, $q, $location, errorInGetNodes = false, getNodesError, errorInGetPath = false, getPathError,
	currentProject = {
	...
	beforeEach(function(){
		...
		inject(function($controller, _$rootScope_, $mdDialog, projectService, _$location_, _$q_, presentationService){
	...
	//test
	it("Dovrebbe invocare il metodo $location.path, con il parametro '/paths'", function(){
		$scope.quitPresentation();
		expect($location.path).toHaveBeenCalledWith('/paths');
	});

	it("Dovrebbe impostare il valore dell'attributo $scope.fullscreen a false, nel caso si trovi settato a true", function(){
		$scope.fullscreen = true;
		$scope.quitPresentation();
		expect($scope.fullscreen).toBe(false);
	});

	it("Dovrebbe invocare il metodo $scope.$emit, con il parametro 'premi-fullscreen-off', nel caso si trovi settato a true", function(){
		$scope.fullscreen = true;
		$scope.quitPresentation();
