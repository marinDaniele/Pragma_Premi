 ...
 * Data: 2015-06-08
 * Descrizione: Test di integrazione TI5 Premi::Front-End::Services::MindmapService metodo "addNode"
 */
'use strict';
describe('TI5 Premi::Front-End::Services::MindmapService metodo "addNode"', function () {
	...
	beforeEach(function(){
		...
		spyOn(window, 'Node').and.callThrough();
	});
	...
	it("Dovrebbe invocare il metodo mindmapAdapterService.addNode, passando come parametri parentNodeId e node1", function(){
		mindmapService.addNode(parentNodeId);
		$httpBackend.flush();
        expect(mindmapAdapterService.addNode).toHaveBeenCalledWith(parentNodeId, jasmine.any(Node));
	});
