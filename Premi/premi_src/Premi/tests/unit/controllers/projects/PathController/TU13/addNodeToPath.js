...
describe('TU13 - addNodetoPath()', function() {
    var req, res;
    ...
    before(function (done) {
        req = function (pId, nId, mode) {
            ...
            var outer = this;
            this.project.addNodeToPath = function(nId,pId,callback,errback)
            {
                if(self.mode == 'callback')
                    callback(outer.project.paths[0]);
                else
                    errback(1);
    ...
    it('Dovrebbe eseguire correttamente ed aggiunger il nodo al percorso', function (done) {
        var request = new req('555c3bfe7e9caf422910868e','555c3bfe7e9caf422910868e','callback');
        var response = new res();
        PathCtrl.addNodeToPath(request, response, function (err) {
            if (err)
                response.error = 1;
        });
