...
describe('TU13 - addPath()', function() {
    var req, res;
    ...
    before(function (done) {
        req = function (pId,name,mode) {
            ...
            this.project.addPath = function(name,callback,errback)
            {
    ...
    it('Dovrebbe fallire quando viene passato un nome vuoto', function (done) {
        var request = new req('','callback');
        var response = new res();
        PathCtrl.addPath(request, response, function (err) {
            if (err)
                response.error = 1;
        });
    ...
    it('Dovrebbe eseguire correttamente quando viene passato un nome valido', function (done) {
        var request = new req('555c3bfe7e9caf422910868e','new path','callback');
        var response = new res();
        PathCtrl.addPath(request, response, function (err) {
            if (err)
                response.error = 1;
