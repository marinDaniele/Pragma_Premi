...
In particolare, il \gloxy{team} ha individuato la necessità di fissare obiettivi, metriche e strategie per i processi più importanti (tratti da ISO/IEC 12207:2008) ed adottare una strategia di misurazione costante della loro qualità, tale da ottenere un tracciamento continuo del soddisfacimento degli obiettivi fissati.
A questo scopo sono stati valutati vari modelli e standard per la misurazione della qualità dei processi, come CMMI e ISO/IEC 15504, ma si è deciso in fine di adottare il modello \textbf{\gloxy{CMM}} (\textit{Capability Maturity Model}) in quanto, basandosi su un minor numero di parametri, permette una valutazione più semplice ed adatta ad un \gloxy{team} con poca esperienza come \gruppo.
...
\end{itemize}
Al fine di assicurare qualità nei processi adottati, il \gloxy{team} ha stabilito le seguenti soglie di riferimento per il modello \gloxy{CMM}:
\begin{itemize}
...
\end{itemize}
Per garantire una buona qualità di processo, il \gloxy{team} ha individuato dallo standard ISO/IEC 12207:2008 i processi che ritiene più importanti nell'arco del ciclo di vita del prodotto e li ha istanziati individuando obiettivi e metriche coerenti con i livelli di qualità perseguiti.
\subsection{Infrastructure Management Process (6.2.2)}
\label{infraMgmt}
Il processo si pone come obiettivo quello di fornire, mantenere ed aggiornare l'infrastruttura ed i servizi necessari allo svolgimento del \gloxy{progetto} nell'arco di tutto il suo ciclo di vita. Con il termine \textit{infrastruttura} si intendono elementi \gloxy{hardware}, software, metodi, strumenti, tecniche e standard impiegati nello sviluppo del prodotto.
\subsubsection{Obiettivi di qualità}
Per tutta la durata del \gloxy{progetto}, l'infrastruttura impiegata nello sviluppo dovrà raggiungere determinati obiettivi; in particolare:
\begin{itemize}
\item tutte le procedure riguardanti le attività svolte più frequentemente durante lo sviluppo del \gloxy{progetto} saranno descritte esaustivamente nel documento \NP;
\item tutti i riferimenti normativi e informativi saranno completi di informazioni utili al loro reperimento;
\item la \gloxy{piattaforma} \pragmadb sarà disponibile all'uso ogniqualvolta un componente del \gloxy{team} avesse bisogno di accedere ai dati in essa contenuti;
\item i dati ottenuti da \pragmadb saranno sempre coerenti e aggiornati;
\item nell'arco dello sviluppo del \gloxy{progetto}, \pragmadb si estenderà fornendo nuove funzionalità in relazione alle nuove attività intraprese; esse risulteranno implementate e funzionanti alla prima necessità di utilizzo;
...
\subsubsubsection{Disponibilità \pragmadb}
\label{dispPragmaDB}
Indica la percentuale di disponibilità di utilizzo della \gloxy{piattaforma} \pragmadb rispetto alle richieste di accesso.
\begin{itemize}
\item \textbf{Misurazione}: $ D = \frac{A}{R} \cdot 100 $, dove $A$ corrisponde al numero di accessi avvenuti correttamente alla pagina di login della \gloxy{piattaforma} e $R$ il numero totale di richieste di accesso alla pagina di login inoltrate alla \gloxy{piattaforma};
\item \textbf{Valori di ottimalità}: $90 - 100$;
\item \textbf{Valori di accettazione}: $80 - 100$.
\end{itemize}
\subsubsubsection{Tempo di correzione incoerenze in \pragmadb}
\label{tCorrIncoerPragmaDB}
Indica il periodo medio intercorso fra l'individuazione di un'incoerenza nella \gloxy{piattaforma} \pragmadb da parte di un verificatore ed il suo aggiornamento da parte di un altro componente del \gloxy{team}.
\begin{itemize}
...
\end{itemize}
%\subsubsubsection{Tempo di implementazione funzionalità \pragmadb}
\subsubsubsection{Errori di individuazione termini di glossario}
...
L'infrastruttura necessaria allo svolgimento del \gloxy{progetto} dovrà essere mantenuta costantemente aggiornata; in particolare l'utilizzo delle metriche sopra indicate permetterà l'individuazione di eventuali errori all'interno degli strumenti utilizzati, la cui correzione (da effettuarsi nel minor tempo possibile) permetterà di ripristinare l'erogazione di dati corretti e coerenti.
\subsection{Project Planning, Assessment \& Control Process (6.3.1 - 6.3.2)}
\label{projPlanAssControl}
Il macro-processo (derivante dall'unione dei processi \textit{Project Planning Process} e \textit{Project Assessment \& Control Process}) ha lo scopo di produrre dei piani di sviluppo per il \gloxy{progetto}, comprendenti scelta del modello di ciclo di vita  del prodotto, descrizioni delle attività e dei compiti da svolgere durante tutto il ciclo di sviluppo, pianificazione temporale del lavoro e dei costi da sostenere, allocazione di compiti e responsabilità, e di effettuare periodicamente delle misurazioni per rilevare lo stato del \gloxy{progetto} rispetto alle pianificazioni prodotte.
\subsubsection{Obiettivi di qualità}
L'intero sviluppo del \gloxy{progetto} dovrà seguire la pianificazione prodotta, in particolare:
\begin{itemize}
...
\item il costo necessario allo svolgimento di una fase di \gloxy{progetto} non dovrà eccedere quanto preventivato per tale fase.
\end{itemize}
\subsubsection{Metriche}
\subsubsubsection{Schedule Variance}
\label{scheduleVariance}
Indica se si è in linea, in anticipo o in ritardo rispetto alla schedulazione delle attività di \gloxy{progetto} pianificate.
\begin{itemize}
...
\end{itemize}
\subsubsubsection{Budget Variance}
...
\begin{itemize}
\item \textbf{Misurazione}: $BV = BCWS - ACWP$, dove $BCWS$ è il costo pianificato per realizzare le attività di \gloxy{progetto} alla data corrente e $ACWP$ è il costo effettivamente sostenuto alla data corrente;
\item \textbf{Valori di ottimalità}: $\geq 0$;
\item \textbf{Valore di accettazione}: $\geq 0$.
\end{itemize}
\subsubsection{Strategie}
La pianificazione effettuata dovrà essere aggiornata costantemente durante tutta l'attività di \gloxy{progetto} per essere sempre coerente con la situazione corrente.\\
Qualsiasi eventuale valore negativo a livello di \textit{Schedule Variance} o  \textit{Budget Variance} rilevato in una fase di lavoro dovrà essere assolutamente compensato entro la fine dell'attività di \gloxy{progetto}, in quanto non è assolutamente ammesso eccedere le ore di lavoro finali e il preventivo dei costi finale indicato nella pianificazione.
\subsection{Risk Management Process (6.3.4)}
\label{riskMgmt}
L'obiettivo del processo è quello di identificare, analizzare, trattare e monitorare continuamente i rischi che possono insorgere durante l'intera attività di \gloxy{progetto}.
\subsubsection{Obiettivi di qualità}
Il \gloxy{team} dovrà gestire correttamente i rischi, in particolare:
\begin{itemize}
\item all'inizio dell'attività di \gloxy{progetto}, verranno individuati i principali fattori di rischio riguardanti l'organizzazione delle attività;
...
\end{itemize}
\subsubsection{Metriche}
\subsubsubsection{Rischi non preventivati}
\label{riskNonPrev}
...
\end{itemize}
\subsubsubsection{Efficienza di gestione dei rischi}
\label{effGestRischi}
...
Anche se a basso livello di pericolosità, in caso il rischio si manifestasse, il \gloxy{team} dovrà attuare le contromisure previste al fine di mitigare i suoi effetti ed evitare che la sua pericolosità aumenti.
\subsection{System/Software Requirements Analysis Process (6.4.2 - 7.1.2)}
\label{sySoRequiAna}
Il processo punta a trasformare i requisiti definiti dagli \gloxy{stakeholder} in un set di requisiti tecnici che fungerà da linea guida nella progettazione del sistema.
\subsubsection{Obiettivi di qualità}
I requisiti identificati dal \gloxy{team} dovranno essere gestiti in maniera tale da raggiungere i seguenti traguardi:
\begin{itemize}
...
\item nessun requisito dovrà risultare superfluo o ambiguo agli \gloxy{stakeholder};
...
\end{itemize}
\subsubsubsection{Requisiti desiderabili/facoltativi accettati soddisfatti}
\label{reqAccSodd}
...
\item \textbf{Valori di accettazione}: $100$.
\end{itemize}
\subsubsubsection{Requisiti desiderabili/facoltativi non accettati soddisfatti}
\label{reqNonAccSodd}
...
\end{itemize}
\subsubsection{Strategie}
Tutti i requisiti individuati dovranno essere correttamente inseriti nella \gloxy{piattaforma} \pragmadb, la quale si occuperà di mantenere traccia delle fonti dalle quali derivano, delle modifiche effettuate e della loro implementazione nel prodotto.
...
\subsubsection{Obiettivi di qualità}
Durante lo svolgimento delle attività previste da questo processo, il \gloxy{team} punterà a definire un'architettura adatta agli scopi del \gloxy{progetto}:
\begin{itemize}
...
\end{itemize}
\subsubsection{Metriche}
\subsubsubsection{Structural Fan-In}
...
\item \textbf{Valori di accettazione}: $\geq 0$.
\end{itemize}
\subsubsubsection{SFIN - Ottimalità}
...
\end{itemize}
\subsubsubsection{Structural Fan-Out}
\label{SFOUT}
...
\item \textbf{Valori di accettazione}: $0 - 5$.
\end{itemize}
\subsubsubsection{SFOUT - Non Accettabilità}
\label{sfout-NonAcc}
...
\end{itemize}
\subsubsection{Strategie}
Nel corso dell'attività di progettazione (sia ad alto livello che di dettaglio) le componenti verranno inserite nella \gloxy{piattaforma} \pragmadb, la quale si occuperà di mantenere aggiornati i tracciamenti fra esse ed i requisiti che soddisfano, oltre alle relazioni presenti fra le varie componenti.
...
\end{itemize}
\subsubsection{Metriche}
\subsubsubsection{Numero di metodi per classe}
...
\end{itemize}
\subsubsubsection{Metodi per classe - Non Accettabilità}
...
\end{itemize}
\subsubsubsection{Parametri per metodo - Non Accettabilità}
...
\subsubsection{Obiettivi di qualità}
Le unità software prodotte dovranno risultare di qualità; a questo fine il \gloxy{team} si è posto i seguenti obiettivi:
\begin{itemize}
...
\end{itemize}
\subsubsection{Metriche}
\subsubsubsection{Produttività di codifica}
\label{prodCod}
...
\item \textbf{Valori di accettazione}: $0 - 8$.
\end{itemize}
\subsubsubsection{Complessità Ciclomatica - Non Accettabilità}
\label{complCiclomNA}
...
\end{itemize}
\subsubsubsection{Linee di commento su linee di codice}
\label{lineeCommento}
...
\end{itemize}
\subsubsubsection{Chiamate annidate}
Indica il numero di funzioni o procedure chiamate all'interno di un metodo; un numero eccessivo potrebbe causare dei problemi a livello di \gloxy{stack}.
\begin{itemize}
...
\item \textbf{Valori di accettazione}: $0 - 8$.
\end{itemize}
\subsubsubsection{Variabili inutilizzate}
...
\end{itemize}
\subsubsubsection{Halstead Difficulty per-function}
\label{halDiff}
...
\end{itemize}
\subsubsubsection{Halstead Difficulty per-function - Non Accettabilità}
\label{halDiffNA}
...
\item \textbf{Valori di accettazione}: $0 - 1$.
\end{itemize}
\subsubsubsection{Halstead Volume per-function}
\label{halVolume}
...
\end{itemize}
\subsubsubsection{Halstead Effort per-function}
\label{halEffort}
...
\end{itemize}
\subsubsubsection{Indice di manutenibilità - Non Accettabilità}
\label{indManNA}
...
\begin{itemize}
\item l'integrazione delle varie parti del sistema sarà completamente automatizzata utilizzando lo strumento di \gloxy{continuous integration} \textit{Jenkins};
\item il livello di integrazione raggiunto del sistema sarà sempre consultabile grazie all'utilizzo dello strumento di \gloxy{continuous integration} \textit{Jenkins}.
\end{itemize}
\subsubsection{Metriche}
...
\end{itemize}
\subsubsection{Strategie}
Sarà necessario configurare accuratamente lo strumento di \gloxy{continuous integration} \textit{Jenkins} affinché esegua dei test di integrazione di quanto prodotto prima che le ultime modifiche diventino parte del sistema.
...
\end{itemize}
\subsubsubsection{Test di Validazione eseguiti}
\label{tvalese}
...
\begin{itemize}
\item la documentazione prodotta dovrà essere chiara e comprensibile a tutti gli \gloxy{stakeholder} e sarà resa disponibile alle parti interessate per la consultazione;
...
\begin{itemize}
\item la documentazione verrà verificata attraverso \textit{\gloxy{inspection}}, poiché (effettuando un'analisi mirata degli errori) permette risparmio in termini di tempi e costi;
...
\label{coperturaTest}
Indica la percentuale di \textit{branch coverage} rilevata sul codice, ossia la percentuale di rami decisionali \nogloxy{percorsi} dalla suite di test di unità utilizzata.
\begin{itemize}
\item \textbf{Misurazione}: $BC=\frac{R_{P}}{R_{T}} \cdot 100$, dove $R_{P}$ è il numero dei rami decisionali \nogloxy{percorsi} dai test e $R_{T}$ è il numero di rami decisionali definiti nel software;
\item \textbf{Valori di ottimalità}: $80 - 100$;
\item \textbf{Valori di accettazione}: $70 - 100$.
\end{itemize}
...
\end{itemize}
\subsubsection{Strategie}
Durante le attività di correzione della documentazione, gli errori più frequenti rilevati saranno riportati in un documento, in modo tale da diventare uno dei punti chiave delle successive attività di \textit{\gloxy{inspection}}.\\
